...
public class DreamOS_Core extends JFrame {
	...
	public void update(boolean atLaunch) {
		...
		path = osdata[2] + osdata[0] + programManu + osdata[0] + programName + osdata[0] + ver + osdata[0] + "storage" + osdata[0] + "Latest.signdoc";
		...
		if(LatestVer!=null) {
			if(!LatestVer.equals(ver)) {
				...
				if(OSKind.contains("windows")) {
					...
					sUrl = "https://github.com/NVTechKorea/MessageEncryptor/raw/master/Windows/" + LatestVer + "/MessageEncryptor.zip";
				}else if(OSKind.contains("mac")) {
					print("Downloading for macOS...");
					sUrl = "https://github.com/NVTechKorea/MessageEncryptor/raw/master/macOS/" + LatestVer + "/MessageEncryptor.zip";
				}else {
					print("Downloading Universal...");
					sUrl = "https://github.com/NVTechKorea/MessageEncryptor/raw/master/Universal/" + LatestVer + "/MessageEncryptor.jar";
				}
				if(OSKind.startsWith("windows")||OSKind.startsWith("mac")) {
					path = osdata[2] + osdata[0] + "Downloads" + osdata[0] + "MessageEncryptor_" + LatestVer + "_Update.zip";
				}else {
					path = osdata[2] + osdata[0] + "Downloads" + osdata[0] + "MessageEncryptor_" + LatestVer + "_Update.jar";
				}
